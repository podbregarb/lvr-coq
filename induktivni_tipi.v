...
Proof.
  induction n. (* ce tu napisemo induction n ; auto bo kar uporabil auto na podproblemih -> ni treba v naslednji vrstici napisati auto. *)
  - auto.
  - simpl.
    ring_simplify.
    SearchAbout (?x * ?y + ?x). (* tu dobimo v spodnjem kvadratku rezultate iskanja in pogledamo kaj moramo uporabiti *)
    rewrite <- mult_succ_r.
    congruence. (* ko bi samo se vstavljali enacbe napisemo congruence. in Coq vse naredi sam *)
Qed.
...
Proof.
  intro H.
  destruct t.
  - discriminate. (* pogleda ce so kake brezvezne enacbe in jih "resi" *)
  - destruct t1 ; destruct t2 ; try discriminate || auto. (* || pomeni ali *)
Qed.
